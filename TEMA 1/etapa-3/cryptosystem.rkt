...
(define (key n)
  (let*
      ...
      (l (map (λ (func) (func e f))
        (list a1 b1 c1 a2 b2 c2 a3 b3 c3))))
   (map (λ (x) (modulo x 27)) l)))
...
(define (message->codes message)
  (let
      ((l (map char->integer (string->list message))))
  (map (λ (x) (if (> x 96) (- x 96) 0)) l)
))
...
(define (codes->message codes)
  (let
      ((l (map (λ (x) (if (> x 0) (+ x 96) 32)) codes)))
...
(define (extend-key key size)
  (let*
    ...
    (cond
     ((< dif 0) (take key size))
     ((> dif 0) (extend-key (append key key) size))
     ((= dif 0) key))))
...
(define apply-algorithm
  (λ (f)
    (λ (codes key)
     (map (λ (x y) (modulo (f x y) 27))
          codes (extend-key key (length codes))))))

(define (encrypt-codes message key)
  ((apply-algorithm +) message key))

(define (decrypt-codes crypted key)
  ((apply-algorithm -) crypted key))
...
(define apply-algorithm-letters
  (λ (f)
    (λ (w key)
     (codes->message (f (message->codes w) key)))))

(define (encrypt-message message key)
  ((apply-algorithm-letters encrypt-codes) message key))
(define (decrypt-message crypted key)
  ((apply-algorithm-letters decrypt-codes) crypted key))
